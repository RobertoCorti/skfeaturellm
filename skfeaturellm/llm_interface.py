...
class LLMInterface:  # pylint: disable=too-few-public-methods
    ...
    Args:
        config (Dict[str, Any]): Configuration for the LLM (API keys, model selection, etc.)
    """
    ...
    def generate_feature_ideas(
        ...
        response = self.llm.generate([messages])  # pylint: disable=unused-variable
    ...
    def _prepare_data_description(
        self, data: pd.DataFrame, feature_descriptions: Optional[Dict[str, str]] = None
    ) -> str:
